...
function(a) {
    ...
    c && (a = global, a.JS_MD5_TEST && (a.navigator = {
        userAgent: "Firefox"
    }));
    var h, A = (a.JS_MD5_TEST || !c) && -1 != navigator.userAgent.indexOf("Firefox"),
        g = !a.JS_MD5_TEST && "undefined" != typeof ArrayBuffer,
        y = "0123456789abcdef".split(""),
        v = [128, 32768, 8388608, -2147483648],
        S = [0, 8, 16, 24],
        T = [];
    if (g) {
    ...
    var w = function(a) {
            ...
            var _, w, B, C, D, E, J, d, M, U, b, i, F = !0,
                H = !1,
                O = 0,
                j = 0,
                k = 0,
            ...
            T[16] = 0;
            do {
                if (T[0] = T[16], T[16] = T[1] = T[2] = T[3] = T[4] = T[5] = T[6] = T[7] = T[8] = T[9] = T[10] = T[11] = T[12] = T[13] = T[14] = T[15] = 0, c) if (g) for (i = j; z > O && 64 > i; ++O) h[i++] = a[O];
                else for (i = j; z > O && 64 > i; ++O) T[i >> 2] |= a[O] << S[3 & i++];
                else if (g) for (i = j; z > O && 64 > i; ++O) b = a.charCodeAt(O), 128 > b ? h[i++] = b : 2048 > b ? (h[i++] = 192 | b >> 6, h[i++] = 128 | 63 & b) : 55296 > b || b >= 57344 ? (h[i++] = 224 | b >> 12, h[i++] = 128 | b >> 6 & 63, h[i++] = 128 | 63 & b) : (b = 65536 + ((1023 & b) << 10 | 1023 & a.charCodeAt(++O)), h[i++] = 240 | b >> 18, h[i++] = 128 | b >> 12 & 63, h[i++] = 128 | b >> 6 & 63, h[i++] = 128 | 63 & b);
                else for (i = j; z > O && 64 > i; ++O) b = a.charCodeAt(O), 128 > b ? T[i >> 2] |= b << S[3 & i++] : 2048 > b ? (T[i >> 2] |= (192 | b >> 6) << S[3 & i++], T[i >> 2] |= (128 | 63 & b) << S[3 & i++]) : 55296 > b || b >= 57344 ? (T[i >> 2] |= (224 | b >> 12) << S[3 & i++], T[i >> 2] |= (128 | b >> 6 & 63) << S[3 & i++], T[i >> 2] |= (128 | 63 & b) << S[3 & i++]) : (b = 65536 + ((1023 & b) << 10 | 1023 & a.charCodeAt(++O)), T[i >> 2] |= (240 | b >> 18) << S[3 & i++], T[i >> 2] |= (128 | b >> 12 & 63) << S[3 & i++], T[i >> 2] |= (128 | b >> 6 & 63) << S[3 & i++], T[i >> 2] |= (128 | 63 & b) << S[3 & i++]);
...
}(this); /*!pc/modules/country-code/main.js*/
;
define("pc/modules/country-code/main", ["require", "exports", "module"], function() {
    ...
    $(document).on("click", ".area_code", function() {
        ...
        url: "/register/getPhoneCountryCode.json",
        success: function(c) {
            // var a = "",
            //     h = c.content.rows;
            // if (v.text(h[0].countryList[0].code), 1 === c.state && h) for (var _ = 0, g = h.length; g > _; _++) {
            //     a += "<dt>" + h[_].name + "</dt>";
            //     for (var i = 0, y = h[_].countryList.length; y > i; i++) a += "<dd>" + h[_].countryList[i].name + "<span>" + h[_].countryList[i].code + "</span></dd>"
            // } else a = "请求出错";
...
define("pc/page/login/main", ["require", "exports", "module", "dep/jquery-placeholder/jquery.placeholder", "pc/modules/country-code/main"], function(require) {
    ...
    $(".form_head").on("click", "li", function() {
        var g = $(this),
            v = g.index();
        0 === v ? $(".divider").removeClass("code_divider") : $(".divider").addClass("code_divider"), g.addClass("active").siblings().removeClass("active"), F.stop().animate({
            left: g.offsetParent().context.offsetLeft
        }, 400), a.eq(v).show().siblings(".form_body").hide(), _.setClear(), h.setClear(), clearInterval(c);
        var C = a.eq(v);
            ...
            fields: [{
                ...
                click: function(e) {
                    ...
                    F.isValidate && (F.password = md5(F.password), F.password = md5(c + F.password + c), $.ajax({
                        ...
                        g[F.state] ? ($('[data-propertyname="' + g[F.state].linkFor + '"] input').blur(), a.parent.field[g[F.state].linkFor].showMessage({
                            message: g[F.state].message
                        })) : ($('[data-propertyname="password"] input').blur(), a.parent.field.password.showMessage({
                            message: F.message
        ...
        fields: [{
            ...
            click: function(e) {
                ...
                (-1 == a.control.totalTimeTemp || a.control.totalTimeTemp == a.control.getTopTime()) && $.ajax({
                    ...
                    var c;
                    v[F.state] ? a.parent.field[v[F.state].linkFor].showMessage({
                        message: v[F.state].message
                    }) : a.parent.field.phoneVerificationCode.showMessage({
                        message: F.message
                    }), 1 == F.state ? (c = $('[data-propertyname="phoneVerificationCode"]'), $(".verify_tips_main").hide(), e.control.starttime(e.control, function() {
            ...
            click: function(e) {
                ...
                (-1 == a.control.totalTimeTemp || a.control.totalTimeTemp == a.control.getTopTime()) && $.ajax({
                    ...
                    }, 1e3)) : (e.control.init(), _ ? a.parent.field[_.linkFor].showMessage({
                        message: _.message
                    }) : a.parent.field.phoneVerificationCode.showMessage({
                        message: F.message
            ...
            click: function(e) {
                ...
                F.countryCode = $('[data-view="codeLogin"] .area_code').text(), F.isValidate && $.ajax({
                    ...
                    g[F.state] ? ($('[data-propertyname="' + g[F.state].linkFor + '"] input').blur(), a.parent.field[g[F.state].linkFor].showMessage({
                        message: g[F.state].message
                    })) : ($('[data-propertyname="phoneVerificationCode"] input').blur(), a.parent.field.phoneVerificationCode.showMessage({
                        message: F.message
                    })), a.parent.field.request_form_verifyCode.getVerifyCode()
                })
            }
